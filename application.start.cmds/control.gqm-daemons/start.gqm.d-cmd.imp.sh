#!/bin/bash
# ------------------------------------------------------------------
# [Georgios Fisaris]    start.gqm.d:cmd:importer
# ------------------------------------------------------------------

VERSION=0.1.0
SUBJECT=aws.ec2.ssm-startGQM_CMD_IMP
...
daemonName="GQM:Command:Importer"
daemonCode="gqm.d:cmd:importer"
daemonSTOPFile=".stop.gqm-cmdimp.service"
...
if [ ! -f "$workingDir$daemonSTOPFile" ]
  then
	...
	cd $workingDir
	php $consoleFile --env=prod $daemonCode 2>> var/logs/gemv2.gqm-cmd-importer.errors.logs >> var/logs/gemv2.gqm-cmd-importer.output.logs &
