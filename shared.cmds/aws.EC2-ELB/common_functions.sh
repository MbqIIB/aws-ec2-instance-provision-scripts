...
if [ "$APPPUBNET" = "yes" ];
	then
    ELB_LIST="EC2-ELB-$APPENV-$APPGROUP-$APPUSER EC2-iELB-$APPENV-$APPGROUP-$APPUSER"
    echo "EC2.ELB: $ELB_LIST" >> /aws.services/.ec2Instance
    else
    ELB_LIST="EC2-iELB-$APPENV-$APPGROUP-$APPUSER"
    echo "EC2.ELB: $ELB_LIST" >> /aws.services/.ec2Instance
fi
